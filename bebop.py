...
def VJPerimeterSearch(robot):
    ...
    try:
        ...
        robot.trim()
        robot.takeoff()
        robot.flyToAltitude(100)

        robot.wait( 5.0 )
        for i in xrange(3):
            robot.wait( 5.0 )
            robot.update( cmd=movePCMDCmd( True, 0, BASE_MOVE, 0, 0 ) )
            robot.wait( 1.0 )
            ...
            print "Next Phase"
            robot.wait( 1.0 )
            robot.update( cmd=movePCMDCmd( True, 0, BASE_FULLMOVE, 0, 0 ) )
            print "1st Phase complete"
            robot.update( cmd=movePCMDCmd( True, 0, 0, LEFT_TURN, 0 ) )
            robot.wait( 1.0 )
            robot.update( cmd=movePCMDCmd( True, 0, BASE_FULLMOVE, 0, 0 ) )
            print "2nd Phase complete"
            robot.update( cmd=movePCMDCmd( True, 0, 0, LEFT_TURN, 0 ) )
            robot.wait( 1.0 )
            robot.update( cmd=movePCMDCmd( True, 0, BASE_FULLMOVE, 0, 0 ) )
            print "3rd Phase comple"
            robot.update( cmd=movePCMDCmd( True, 0, 0, LEFT_TURN, 0 ) )
            robot.wait( 1.0 )
            robot.update( cmd=movePCMDCmd( True, 0, BASE_FULLMOVE, 0, 0 ) )
            print "4th Phase complete"
            robot.update( cmd=movePCMDCmd( True, 0, 0, LEFT_TURN, 0 ) )
            robot.wait( 1.0 )
            robot.update( cmd=movePCMDCmd( True, 0, BASE_MOVE, 0, 0 ) )
            robot.update( cmd=movePCMDCmd( True, 0, 0, LEFT_TURN, 0 ) )
